...
# recorridos ----
db1.01 <- subset(db1, recorrido=='Diurno 1')
head(db1.01)
dim(db1.01)

db1.01$c.spp <- as.character(db1.01$c.spp)
table(db1.01$c.spp)

db1.01$nombre_cientifico_avesdechile <- as.character(db1.01$nombre_cientifico_avesdechile)
table(db1.01$nombre_cientifico_avesdechile)




db2.1 <- db1.01[,c('n.est', 'nombre_cientifico_avesdechile')]
sort(unique(db2.1$n.est))
db2.1$presencia <- 1
head(db2.1)
str(db2.1)

table(db2.1$nombre_cientifico_avesdechile)
table(db2.1$n.est)

num <- sort(unique(db2.1$n.est)) ; num

for (i in num) {
  out.pre <- subset(db2.1, n.est==i)
...
for (i in num) {
  ...
  ej <- data.frame(name=names(table(db2.1$nombre_cientifico_avesdechile)), num=0)
...
#
abun <- as.vector(unlist(table(db2.1$n.est)))
db.plot1 <- data.frame(var=num, val=abun)

ggplot(db.plot1, aes(var, val)) + geom_bar(stat = "identity") + labs(title = 'Recorrido diurno 1', 
...
ggplot(db.plot2, aes(var, val)) + geom_bar(stat = "identity") + labs(title = 'Recorrido diurno 1', 
...
# recorridos ----
db1.01 <- subset(db1, recorrido=='Nocturno 1')
head(db1.01)
dim(db1.01)
...
ggplot(db.plot1, aes(var, val)) + geom_bar(stat = "identity") + labs(title = 'Recorrido nocturno 1', 
       x = "Estación de muestreo", y = "Abundancia")# + theme(axis.text.x = element_text(angle = 0, hjust = 0))
...
ggplot(db.plot2, aes(var, val)) + geom_bar(stat = "identity") + labs(title = 'Recorrido nocturno 1', 
       x = "Estación de muestreo", y = "Riqueza")# + theme(axis.text.x = element_text(angle = 0, hjust = 0))
