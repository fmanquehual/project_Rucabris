...
# recorridos ----
db1 <- subset(db1, numero_recorrido==1 | numero_recorrido==2 | numero_recorrido==3)
head(db1)
dim(db1)

db1$numero_recorrido <- as.numeric(db1$numero_recorrido)
table(db1$numero_recorrido)
...
db2 <- db1[,c('numero_recorrido', 'nombre_cientifico_avesdechile')]
sort(unique(db2$numero_recorrido))
db2$presencia <- 1
head(db2)
str(db2)

table(db2$nombre_cientifico_avesdechile)
table(db2$numero_recorrido)

num <- sort(unique(db2$numero_recorrido)) ; num

# de dataframe a matriz ----

for (i in num) {
  out.pre <- subset(db2, numero_recorrido==i)
...
#
# abun <- as.vector(unlist(table(db2$numero_recorrido)))
# db.plot1 <- data.frame(var=num, val=abun)
# 
# ggplot(db.plot1, aes(var, val)) + geom_bar(stat = "identity") + labs(title = 'Recorrido diurno',
...
db.plot2$var <- as.character(db.plot2$var)
str(db.plot2)


col.p <- c('#FF0080','#3ADF00', '#FFBF00' ) # colores de leyenda

p <- ggplot(db.plot2, aes(var, val, fill=var)) + geom_bar(stat = "identity") + labs(title = 'Recorrido diurno', 
       x = "Estación de muestreo", y = "Riqueza")

p + scale_fill_manual('Recorridos', values=col.p) # + theme(axis.text.x = element_text(angle = 0, hjust = 0))
...
# Preparacion de datos nocturnos ----
...
# recorridos ----
db1 <- subset(db1, numero_recorrido==1 | numero_recorrido==2)
head(db1)
dim(db1)
...
# db.plot1 <- data.frame(var=num, val=abun)
# 
# ggplot(db.plot1, aes(var, val)) + geom_bar(stat = "identity") + labs(title = 'Recorrido nocturno',
#        x = "Estación de muestreo", y = "Abundancia")# + theme(axis.text.x = element_text(angle = 0, hjust = 0))
...
ggplot(db.plot2, aes(var, val)) + geom_bar(stat = "identity") + labs(title = 'Recorrido nocturno', 
